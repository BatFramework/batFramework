...
class TilePicker(CustomBaseScene):
    def __init__(self) -> None:
        ...
        self.tileset_image.set_position(
            self.hud_camera.rect.right - self.tileset_image.rect.w, 0
        )
        self.add_hud_entity(self.tileset_image)
        self.add_hud_entity(
            bf.Label("TILE PICKER").set_center(
                *self.hud_camera.rect.move(0, -10).midbottom
            )
        )
        self.picker = bf.Entity(
            (gconst.TILE_SIZE, gconst.TILE_SIZE), convert_alpha=True
        )
        self.picker.set_position = lambda x, y, picker=self.picker: set_pos_picker(
            x,
            y,
            picker,
            self.get_sharedVar("brush_tile"),
            self.tileset_image.rect.topleft,
        )
        ...
        self.add_action(
            bf.Action("l_click").add_mouse_control(1),
            bf.Action("resume").add_key_control(pygame.K_ESCAPE, pygame.K_q),
        ...
        self.timer2 = bf.Time().timer(
            ...
            lambda: pygame.draw.rect(
                self.picker.surface,
                (colors[(next(incrementer2) % len(colors))]),
                (0, 0, *self.picker.rect.size),
                1,
            ),
        )
        ...
        for tag in gconst.TAGS:
            t = bf.Toggle(
                tag,
                callback=lambda x, tag_name=tag: self.get_sharedVar(
                    "brush_tile"
                ).add_tag(tag_name)
                if x
                else self.get_sharedVar("brush_tile").remove_tag(tag_name),
            )
    ...
    def cycle_picker_color_index(self):
        ...
        pygame.draw.rect(
            self.picker.surface,
            colors[self.picker_color_index],
            (0, 0, *self.picker.rect.size),
            1,
        )
    ...
    def do_handle_event(self, event):
        ...
        if self._action_container.is_active("l_click"):
            ...
            x = x // 8
            y = y // 8
            if self.tileset.get_tile(x_index, y_index) == None:
                return
