...
class EditorScene(CustomBaseScene):
    ...
    def do_when_added(self):
        ...
        self.debugger.add_dynamic_data(
            "tags",
            lambda : "NaN" if self.level == None else ''.join([t[0] for t in self.get_tile_tags(self.get_tile_at_mouse())]) )
        
        self.debugger.add_dynamic_data(
            "flip",
            lambda: "Nan" if (self.level is None or self.get_tile_at_mouse() is None) else self.get_tile_at_mouse().flip)
    ...
    def get_tile_at_mouse(self):
        if self.level == None : return None
        return self.level.get_tile_at(*tools.world_to_grid(*[int(i) for i in self.camera.convert_screen_to_world(*pygame.mouse.get_pos())]))
    
    def on_enter(self):
        shared = self.get_sharedVar("level")
        if self.level != shared:
            if self.level != None : self.remove_world_entity(self.level)
            self.level = shared
            self.add_world_entity(self.level)
        if self.manager._scenes[1]._name == "game" : self.camera.set_center(*self.get_sharedVar("game_camera").rect.center)
    ...
    def do_handle_event(self, event):
        ...
        if self._action_container.is_active("save"):
            ...
            bf.Time().timer(duration=400,callback=lambda : self.notif_label.set_visible(False)).start()
        ...
        if self._action_container.is_active("l_click"):
            x, y = tools.world_to_grid(*[int(i) for i in self.camera.convert_screen_to_world(*pygame.mouse.get_pos())])
            ...
            match self.tool:
                ...
                case "entity":
                        if not self.level.get_neighboring_entities(*self.camera.convert_screen_to_world(*pygame.mouse.get_pos()),1):
                            self.level.add_entity(brush_tile.copy().set_position(int(x*gconst.TILE_SIZE),int(y*gconst.TILE_SIZE)))
        ...
        if self._action_container.is_active("r_click"):
            ...
            match self.tool:
                ...
                case "entity":
                    tile =self.level.get_neighboring_entities(*self.camera.convert_screen_to_world(*pygame.mouse.get_pos()),1)
                    if tile: self.level.remove_entity(tile[0])
