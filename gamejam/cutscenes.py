...
class DialogueBlock(bf.CutsceneBlock):
    def __init__(
        self, message, character=None, emotion=None, delay=1000, facing_right=True
    ) -> None:
        super().__init__()
        ...
        self.timer = bf.Time().timer(
            duration=delay,
            callback=lambda: bf.CutsceneManager().manager.get_scene("dialogue").say(self.message, callback=self.end),
        )

    def start(self):
        super().start()
        bf.CutsceneManager().manager.get_scene("dialogue").set_sprite(
...
class ImageBlock(bf.CutsceneBlock):
    ...
    def start(self):
        super().start()
        bf.CutsceneManager().manager.get_scene("dialogue").set_background(
            self.image_path
        )
        self.timer.start()
    ...
    def end(self):
        if self.hide_at_exit:
            bf.CutsceneManager().manager.get_scene("dialogue").set_background(None)
        return super().end()
    ...
    def __init__(self) -> None:
        ...
        self.add_block(
            ...
            bf.SceneTransitionBlock(
                scene="dialogue",
                transition=bf.FadeColorTransition,
                duration=300,
                color=bf.color.DARK_GB,
            ),
            DialogueBlock("So here's the thing...", "player"),
            DialogueBlock(
                "I didn't have enough time to make the story or good levels...",
                "player",
                "thinking",
            ),
            ...
            DelayBlock(1000),
            DialogueBlock(
                "Press the 'e' key to switch to edit mode !", "baby", "happy"
            ),
            DelayBlock(1000),
