...
class Tile(bf.AnimatedSprite):
    ...
    def update_surface(self):
        self.surface = bf.utils.get_tileset("tileset").get_tile(
            *self.tile_index, *self.flip
        )
...
def one_way_up_collideY(self:Tile,collider:bf.Entity):
    return collider.velocity.y > 0 and collider.rect.bottom < self.rect.centery

def wave_update(self: Tile,dt: float):
        Tile.update(self,dt)
        self.set_position(
            self.rect.x,
            self.anchor_y + (2 * sin(self.offset + pygame.time.get_ticks() * 0.01)),
        )
def always_return_true(*args, **kwargs):
    return True

def always_return_false(*args, **kwargs):
    return False
...
def customize_tile(self: Tile):
    ...
    if self.has_tag("collider"):
        self.on_collideX = self.on_collideY = always_return_true
        if self.has_tag("one_way_up"):
            self.on_collideY = functools.partial(one_way_up_collideY,self)
            self.on_collideX = always_return_false
    if self.has_tag("wave"):
        self.anchor_y = self.rect.top
        self.offset = pygame.time.get_ticks() * random.randint(-100, 100) * 0.01

        self.update = functools.partial(wave_update,self)
    if self.has_tag("bounce"):
        self.process_event = (
            lambda event: bf.AudioManager().play_sound("jump")
            if event.type == gconst.STEP_ON_EVENT and event.entity.is_equal(self)
            else 0
        )
...
class Chunk(bf.Entity):
    ...
    def serialize(self) -> dict:
        return {
            ...
            "tiles": {str(key): tile.serialize() for key, tile in self.tiles.items()},
        }
    ...
    def remove_tile(self, x, y):
        tile_pos_x, tile_pos_y = x % gconst.CHUNK_SIZE, y % gconst.CHUNK_SIZE
        if (tile_pos_x < 0 or tile_pos_y < 0) or (
            tile_pos_x,
            tile_pos_y,
        ) not in self.tiles:
            return False
    ...
    def update_surf(self):
        ...
        self.debug_surface = self.surface.copy()
        for t in self.get_tiles():
            if t.has_tag("collider"):
                pygame.draw.rect(
                    self.debug_surface,
                    t._debug_color,
                    (t.rel_x, t.rel_y, *t.rect.size),
                    1,
                )
        self.dirty = False
        pygame.draw.rect(
            self.debug_surface, self._debug_color, (0, 0, *self.rect.size), 1
        )
        return i
...
class Level(bf.Entity):
    ...
    def set_background_image(self, path):
        self.background_image = pygame.image.load(
            join(bf.const.RESOURCE_PATH, path)
        ).convert()
    ...
    def serialize(self) -> dict:
        return {
            "chunks": [c.serialize() for c in self.chunks.values()],
            "entites": [e.serialize() for e in self.entities],
        }
    ...
    def get_by_tag(self, tag):
        return [
            e
            for e in itertools.chain(list(self.get_all_tiles()), self.get_entites())
            if e.has_tag(tag)
        ]
    ...
    def draw(self, camera: bf.Camera) -> bool:
        ...
        if show_debug:
            ...
            for entity in self.get_neighboring_entities(
                *camera.rect.center, camera.rect.w // 2
            ):
                pygame.draw.rect(
                    camera.surface,
                    entity._debug_color,
                    camera.transpose(entity.rect),
                    1,
                )
