...
def _toggle_compute_size(self:bf.Toggle):
    # print("TOGGLE COMPUTE SIZE")
        ...
        new_rect_size[0] += self._padding[0]*2 +TOGGLE_INDICATOR_SIZE[0] *3 #+ self._border_radius[0] // 2 # +(2 if self._outline else 0)
...
def stylize(e : bf.Entity):
    match e:
        ...
        case bf.TextBox():
            e.set_background_color(bf.color.DARK_GB).set_border_width(1).set_border_color(bf.color.SHADE_GB).set_padding(DEFAULT_PADDING)
            stylize(e.label)
        ...
        case bf.Container():
            ...
            e.set_border_width(2).set_border_color(bf.color.LIGHT_GB).set_background_color(bf.color.DARK_GB).set_padding((6,6)).set_gap(4)
            e.update_content()
