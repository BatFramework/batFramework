...
class BootScene(CustomBaseScene):
    def __init__(self) -> None:
        ...
        self.timer = bf.Timer(
            name="boot_timer",
            duration=2000,
            end_callback=lambda: self.manager.transition_to_scene(
                "title",
        ...
        self.add_action(
            bf.Action("skip").add_key_control(
                pygame.K_RETURN, pygame.K_ESCAPE, pygame.K_SPACE
            )
        )
        self.anim = bf.EasingAnimation("fade_start",
            easing_function=bf.Easing.EASE_IN,duration= 2000, end_callback=self.timer.start
        )
    ...
    def do_final_draw(self, surface):
        print(self.anim.get_value())
        self.color_surf.set_alpha(255 - int(255 * self.anim.get_value()))
