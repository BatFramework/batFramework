...
class TitleScene(CustomBaseScene):
    ...
    def do_when_added(self):
        ...
        title_image = bf.Image("backgrounds/title.png").set_center(
            *self.hud_camera.rect.move(0, -bf.const.RESOLUTION[1] // 4).center
        )
        ...
        bf.Button(
            "OPTIONS",
            callback=lambda: self.manager.transition_to_scene(
                "options", bf.FadeTransition
            ),
        ).put_to(main_frame)
        bf.Button(
            "QUIT", callback=lambda: pygame.event.post(pygame.Event(pygame.QUIT))
        ).put_to(main_frame)
