...
import pygame
from level import Level,world_to_grid,grid_to_chunk,grid_to_world
...
class Player(bf.AnimatedSprite):
    def __init__(self) -> None:
        ...
        down = bf.Action("down")
        down.add_key_control(pygame.K_DOWN, pygame.K_DOWN)
        down.set_holding()

        spawn = bf.Action("spawn")
        spawn.add_key_control(pygame.K_s)
        spawn.set_instantaneous()

        self.action_container.add_action(right)
        self.action_container.add_action(left)
        self.action_container.add_action(up)
        self.action_container.add_action(down)
        self.action_container.add_action(spawn)

        self.add_animState("idle","animation/idle.png",(16,16),[10, 10, 10, 10])
        self.add_animState("run","animation/run.png",(16,16),[5, 5, 5, 5])
        self.add_animState("jump","animation/jump.png",(16,16),[3,999])
        self.add_animState("fall","animation/fall.png",(16,16),[10,10])
        self.add_animState("hide","animation/hide.png",(16,16),[3, 3, 10, 4, 5, 3, 999])
    ...
    def process_physics(self,dt):
        ...
        near_tiles = self.level_link.get_neighboring_tiles(*world_to_grid(*[int(i) for i in self.collision_rect.center]))
        if any(near_tiles):
