...
class SceneLayer:
    ...
    def __init__(self, name: str, convert_alpha: bool = False):
        ...
        self.entities_to_add: set[Entity] = (
            set()
        )  # entities to add to the scene, (1 frame delay after calling add)
        self.entities_to_remove: set[Entity] = (
            set()
        )  # entities to remove from the scene
        self.draw_order: list[int] = (
            []
        )  # stores the uid of entities to draw (in draw order)
    ...
    def process_event(self, event: pygame.Event):
        if (
            self.camera.fullscreen
            and event.type == pygame.VIDEORESIZE
            and not pygame.SCALED & bf.const.FLAGS
        ):
    ...
    def draw(self, surface: pygame.Surface):
        ...
        for uid in self.draw_order:
            if (
                uid in self.entities and not self.entities[uid].drawn_by_group
    ...
    def _update_draw_order(self):
        self.draw_order = sorted(
            (
                ...
                if isinstance(v, Drawable) and not v.drawn_by_group
            ),
    ...
    def debug_entity(self, uid: int):
        ...
        def draw_rect(data):
            ...
            if self.camera.intersects(rect):
                line_width = (
                    1
                    if self.camera.zoom_factor >= 1
                    else int(-math.log2(self.camera.zoom_factor)) + 2
                )
                pygame.draw.rect(
                    self.camera.surface,
                    color,
                    self.camera.world_to_screen(rect),
                    line_width,
                )
