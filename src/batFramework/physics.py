...
class Collision:
    ...
    def mtv(self) -> Vector2:
        ...
        return Vector2(self.overlap.x * self.normal.x, self.overlap.y * self.normal.y)
...
class PhysicsWorld(Entity):
    ...
    def remove_layer(self, name: str) -> Self:
        ...
        if name in self.collision_layers:
            ...
            self.layer_matrix = {pair for pair in self.layer_matrix if name not in pair}
        return self

    def set_layers_collide(
        self, layer_a: str, layer_b: str, collide: bool = True
    ) -> Self:
    ...
    def set_collision_callback(
        self, layer_a: str, layer_b: str, callback: Callable[[Collision], None]
    ) -> Self:
    ...
    def aabb_check(a: pygame.FRect, b: pygame.FRect) -> Collision | None:
        ...
        return Collision(entity_a=None, entity_b=None, overlap=overlap, normal=normal)

    def check_collision(
        self, entity_a: "bf.Entity", entity_b: "bf.Entity"
    ) -> Collision | None:
        """
    ...
    def _get_collision_candidates(
        self, entity: "bf.Entity", layer: CollisionLayer
    ) -> Iterator["bf.Entity"]:
    ...
    def _trigger_callback(self, layer_a: str, layer_b: str, collision: Collision):
    ...
    def _resolve_axis(
        ...
        for other_layer_name, other_layer in self.collision_layers.items():
            ...
            for other in self._get_collision_candidates(entity, other_layer):
                ...
                other_is_dynamic = isinstance(other, bf.DynamicEntity) and not getattr(
                    other, "ignore_collisions", False
                )
                ...
                if axis == "x" and collision.overlap.x > 0:
                    ...
                    if other_is_dynamic:
                        ...
                        entity.set_position(
                            entity.rect.x + mtv_x * (mass_b / total), None
                        )
                        other.set_position(
                            other.rect.x - mtv_x * (mass_a / total), None
                        )
                    ...
                    if other_is_dynamic:
                        ...
                        if getattr(other, "grounded", False):
                            ...
                            entity.set_position(
                                None, entity.rect.y + mtv_y * (mass_b / total)
                            )
                            other.set_position(
                                None, other.rect.y - mtv_y * (mass_a / total)
                            )
    ...
    def query_rect(
        self, rect: pygame.FRect, layers: list[str] | None = None
    ) -> list["bf.Entity"]:
        """
    ...
    def query_point(
        self, point: tuple[float, float], layers: list[str] | None = None
    ) -> list["bf.Entity"]:
        """
...
def find_collisions(
    entity: "bf.Entity", candidates: list["bf.Entity"]
) -> list["bf.Entity"]:
    """Find all entities from candidates that collide with entity"""
    return [
        e for e in candidates if e is not entity and entity.rect.colliderect(e.rect)
    ]
