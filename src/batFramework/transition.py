...
class Transition:
    ...
    def update(self,progression:float)->None:
        if self.update_callback: self.update_callback(progression)
    ...
    def skip(self,no_callback :bool = False):
        self.controller.stop()
        if self.end_callback  and (no_callback == False): self.end_callback()


class FadeColor(Transition):
    def __init__(self, color : tuple|str ,middle_duration: float, first_duration : float = None, second_duration:float = None,easing_function: bf.easing = bf.easing.LINEAR) -> None:
        super().__init__(0, easing_function)
        if first_duration is None : first_duration = middle_duration
        if second_duration is None : second_duration = middle_duration
        self.index = 0
        self.first = Fade(first_duration)
        self.color = color
        self.second = Fade(second_duration).set_end_callback(lambda : self.next_step(self.end))
        self.timer = bf.Timer(middle_duration,lambda : self.next_step(self.second.start))
    ...
    def skip(self,no_callback : bool = False):
        if (no_callback == False) and self.end_callback : self.end_callback() 
