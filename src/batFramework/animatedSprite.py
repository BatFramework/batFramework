...
class AnimatedSprite(bf.Drawable):
    ...
    def __init__(
        self,
        *args,
        sprite_rect_anchor: bf.enums.alignment = bf.enums.alignment.CENTER,
        sprite_rect_offset: tuple[float] = None,
        **kwargs,
    ) -> None:
    ...
    def update(self, dt):
        ...
        setattr(
            self.sprite_rect,
            self.sprite_rect_anchor.value,
            getattr(self.rect, self.sprite_rect_anchor.value),
        )
    ...
    def draw(self, camera):
        if not self.animation:
            return
        self.surface = self._animation.get_frame(
            self._animation.frame_number, *self._flip
        )
        if (
            not self.visible
            or self.drawn_by_group
            or not camera.world_rect.colliderect(self.sprite_rect)
