...
class Button(Label, InteractiveEntity):
    ...
    def activate(self,bypass_focus = False):
        ...
        if (not self._focused) and (not bypass_focus) or self._activate_flash > 0 :
            return
        if bypass_focus and not self._focused:self.get_focus()

        self._activate_flash = self._flash_animation_length
        if self.activate_sfx:
            bf.AudioManager().play_sound(self.activate_sfx)

        if self.parent_container: self.parent_container.lock_focus = True
    ...
    def draw_effect(self,camera):
        ...
        # print(width)
        if int(width) > 0 : pygame.draw.rect(camera.surface, self.feedback_color, camera.transpose(self.rect.inflate(2,2)),int(width),*self._border_radius)

    def draw(self, camera):
        if not self.visible or not camera.intersects(self.rect):return False
        if self._hovering:
            camera.surface.blit(self._hover_surf,camera.transpose(self.rect),None,pygame.BLEND_ALPHA_SDL2)
        else:
            super().draw(camera)
    ...
    def draw_focused(self, camera):
        self.draw(camera)
        focus_width = 2
        pygame.draw.rect(camera.surface, bf.color.RIVER_BLUE, camera.transpose(self.rect.inflate(focus_width*2,focus_width*2)),focus_width,*self._border_radius)
        if self._activate_flash > 0:
            self.draw_effect(camera)