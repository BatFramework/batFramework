...
class Container(Panel,InteractiveEntity):
    ...
    def create_link(source:"Container", dest:"Container", hide_source=False,reset_dest_focus = True):
        def fun():
            ...
            if reset_dest_focus : dest.set_focused_child_index(0)
    ...
    def set_direction(self, direction: bf.Direction):
        ...
        if direction in bf.Direction.__members__.values():
            ...
            print(f"Invalid direction '{direction}'. Using default: {self.direction.name}")
        self.update_content()
        return self
    ...
    def set_alignment(self, alignment: bf.Alignment):
        ...
        if alignment in bf.Alignment.__members__.values():
            ...
            print(f"Invalid alignment '{alignment}'. Using default: {self.alignment.name}")
        self.update_content()
        return self
    ...
    def remove_entity(self, entity:bf.Entity):
        ...
        if isinstance(entity, InteractiveEntity):
            ...
            if len(self.interactive_children) == 0: self._no_focusable_child = True
        self.update_content()
    ...
    def set_focused_child_index(self,index:int):
        if index < 0 or index >= len(self.interactive_children):return False
    ...
    def set_focused_child(self,child:InteractiveEntity):
        try : 
            ...
            return False
        if index != self.focused_index : self.set_focused_child_index(index)
    ...
    def update_content(self):
        ...
        # print(self.rect)
        if not self.surface or self.surface.get_size() != tuple(int(i) for i in self.rect.size): 
            self.surface = pygame.Surface(self.rect.size).convert_alpha()
            self.update_surface()

    def resize(self,new_width,new_height):
        self._manual_resized = True
        if (new_width,new_height) == self.rect.size : return
    ...
    def resize_by_parent(self,new_width,new_height):
        if self._manual_resized or (new_width,new_height) == self.rect.size : return
        bf.Panel.resize_by_parent(self,new_width,new_height)
        self.update_content()

    def resize_by_self(self,new_width,new_height):
        if (new_width,new_height) == self.rect.size : return self
    ...
    def update_horizontal_layout(self):
        ...
        total_width = max(0, total_children_width + total_gap_width + self._padding[0] * 2)

        max_child_height = max(child.rect.h for child in self.children) if self.children else self.rect.h
    ...
    def draw_focused_child(self, camera):
        ...
        if child : child.draw_focused(camera)
    ...
    def draw(self, camera):
        ...
        for child in [c for c in self.children if c.visible  or not isinstance(c, InteractiveEntity) or not c._focused]:
            child.draw(camera)
            num_drawn += 1
        if self._focused:
