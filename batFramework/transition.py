...
class BaseTransition:
    def __init__(self,source_surf : pygame.Surface,dest_surf:pygame.Surface,duration=100,**kwargs) -> None:
...
class FadeColorTransition(BaseTransition):
    def __init__(self, source_surf, dest_surf,duration=200,color_duration=200,color=bf.color.CLOUD_WHITE,**kwargs) -> None:
        ...
        self.ease_out = bf.EasingAnimation(bf.Easing.EASE_IN,duration,lambda x: self.color_surf.set_alpha(int(255 - (255 * x))),lambda : self.set_ended(True))

        self.color_timer = bf.Time().timer(name="GOB",duration=color_duration,callback=lambda : [self.set_state("out"),self.ease_out.start()])
        self.ease_in = bf.EasingAnimation(bf.Easing.EASE_IN,duration,update_callback= lambda x: self.color_surf.set_alpha(int(255 * x)),end_callback= lambda : [self.set_state("color"),self.color_timer.start()])
        self.state = "in"
        self.ease_in.start()
...
class SlideTransition(BaseTransition):
    def __init__(self, source_surf, dest_surf,duration=1000,source_alignment : bf.Alignment=bf.Alignment.BOTTOM,easing:bf.Easing=bf.Easing.EASE_IN_OUT,**kwargs) -> None:
        super().__init__(source_surf, dest_surf,duration)
        self.offset = pygame.Vector2(0,0)
        if source_alignment in [bf.Alignment.TOP, bf.Alignment.BOTTOM]:
            self.offset.y = bf.const.RESOLUTION[1]
            if source_alignment == bf.Alignment.TOP : self.offset.y *= -1
        elif source_alignment in [bf.Alignment.LEFT , bf.Alignment.RIGHT]:
            self.offset.x = bf.const.RESOLUTION[0]
            if source_alignment == bf.Alignment.LEFT : self.offset.x *= -1
        else:
            self.offset.x = -bf.const.RESOLUTION[0]
            print(f"Unsupported Alignment : {source_alignment.value}, set to default : {bf.Alignment.LEFT.value} ")
        self.anim = bf.EasingAnimation(easing,duration,lambda x:self.update_offset(self.offset.lerp((0,0),x)),lambda:self.set_ended(True))
        self.anim.start()
