...
class Utils:
    ...
    class Tileset:
        ...
        def split_surface(self,surface: pygame.Surface):
            ...
            for y in range(num_tiles_y):
                for x in range(num_tiles_x):
                    ...
                    tile_surface = surface.subsurface(pygame.Rect(tile_x, tile_y, self.tile_size,self.tile_size))
        ...
        def get_tile(self,x,y,flipX=False,flipY=False) -> pygame.Surface | None:
            if (x,y) not in self.tile_dict : return None
            if flipX or flipY:
                key = f"{x}{y}:{flipX}{flipY}"
                if not key in self._flip_cache:
                    self._flip_cache[key] = pygame.transform.flip(self.tile_dict[(x,y)],flipX,flipY)
    ...
    def img_slice(file, cell_width, cell_height, flipX=False) -> list[pygame.Surface]:
        src = pygame.image.load(os.path.join(bf.const.RESOURCE_PATH, file)).convert_alpha()
    ...
    def load_tileset(path:str,name:str,tilesize):
        if name in Utils.tilesets:
            ...
            img = pygame.image.load(os.path.join(bf.const.RESOURCE_PATH,path)).convert_alpha()
    ...
    def get_tileset(name: str) -> Tileset:
        if name not in Utils.tilesets: return None
        return Utils.tilesets[name]
