...
class AudioManager(metaclass=bf.Singleton):
    ...
    def load_sound(self,name,path,persistent=False) -> pygame.mixer.Sound:
        if name in self.sounds : return self.sounds[name]["sound"]
        path = bf.utils.get_path(path)
        self.sounds[name] = {"path":path,"sound":pygame.mixer.Sound(path),"persistent":persistent}
    ...
    def stop_sound(self,name):
        if name in self.sounds : self.sounds[name]["sound"].stop()
    ...
    def stop_music(self):
        if not self.current_music: return
