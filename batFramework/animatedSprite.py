...
class AnimState:
    def __init__(self,file,width,height,frame_length_list) -> None:
        ...
        self.frames_flipX :list[pygame.Surface] = bf.utils.img_slice(file,width,height,True)
...
class AnimatedSprite(bf.Entity):
    ...
    def add_animState(self,name:str,file:str,size: tuple[int,int],frame_length_list:list[int]):
        if name in self.animStates : return
        self.animStates[name] = AnimState(file,*size,frame_length_list)

    def set_animState(self,state,reset_counter=True,lock = False):
        if state not in self.animStates or self._locked :return False
        self.current_animState = state
        self.rect = self.animStates[self.current_animState].frames[0].get_frect(center=self.rect.center)
        if reset_counter or self.float_counter > sum(self.get_state().frame_length_list) : self.float_counter = 0
        if lock : self.lock_animState()
        return True

    def get_state(self):
        if self.current_animState not in self.animStates:return None
        return self.animStates[self.current_animState]

    def get_frame_index(self):
        return self.animStates[self.current_animState].get_frame_index(self.float_counter)
    ...
    def draw(self, camera: bf.Camera) -> bool:
        if not self.visible or not camera.intersects(self.rect) or not self.animStates: return False
        # pygame.draw.rect(camera.surface,"purple",camera.transpose(self.rect).move(2,2))
        camera.surface.blit(self.get_state().get_frame(self.float_counter,self.flipX), camera.transpose(self.rect))
        return True
